...
describe("CheckoutSession", () => {
  ...
  describe("commit message generation", () => {
    it("returns a trimmed generated message without mutating git", async () => {
      const { checkout, emitted } = makeCheckoutSession({
        gitMetadataGenerator: {
          generateCommitMessage: async () => "  Generated subject  ",
        },
      });

      await checkout.handleCheckoutGenerateCommitMessageRequest({
    ...
    it("returns an error when generation produces no message", async () => {
      const { checkout, emitted } = makeCheckoutSession();

      await checkout.handleCheckoutGenerateCommitMessageRequest({
        type: "checkout.commit_message.generate.request",
        cwd: "/repo",
        requestId: "generate-empty",
      });
      ...
      expect(emitted).toEqual([
        {
          ...
          payload: {
            cwd: "/repo",
            message: null,
            error: { code: "UNKNOWN", message: "Could not generate commit message" },
            requestId: "generate-empty",
          },
        },
      ]);
