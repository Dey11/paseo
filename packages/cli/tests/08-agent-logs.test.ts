...
try {
  // Test 1: logs --help shows options
  {
    ...
    const result = await $`npx hanabicode logs --help`.nothrow();
  ...
  // Test 2: logs requires ID argument
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode logs`.nothrow();
  ...
  // Test 3: logs handles daemon not running
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode logs abc123`.nothrow();
  ...
  // Test 4: logs -f (follow) flag is accepted
  {
    ...
    const result =
      await $`timeout 1 bash -c 'PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode logs -f abc123' || true`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: logs --follow flag is accepted
  {
    ...
    const result =
      await $`timeout 1 bash -c 'PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode logs --follow abc123' || true`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: logs --tail flag is accepted
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode logs --tail 50 abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: logs with ID and --host flag is accepted
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode logs abc123 --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 9: -q (quiet) flag is accepted with logs
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode -q logs abc123`.nothrow();
    const output = result.stdout + result.stderr;
