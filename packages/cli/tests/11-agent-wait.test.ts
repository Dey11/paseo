...
try {
  // Test 1: wait --help shows options
  {
    ...
    const result = await $`npx hanabicode wait --help`.nothrow();
  ...
  // Test 2: wait requires id argument
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode wait`.nothrow();
  ...
  // Test 3: wait handles daemon not running
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode wait abc123`.nothrow();
  ...
  // Test 4: wait --timeout flag is accepted
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx hanabicode wait --timeout 30 --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: wait --host flag is accepted
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx hanabicode wait --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: -q (quiet) flag is accepted with wait
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode -q wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --json flag is accepted with wait
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode wait abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: --format yaml flag is accepted with wait
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode --format yaml wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
