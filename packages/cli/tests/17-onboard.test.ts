...
try {
  console.log("Test 1: `hanabicode` runs blocking onboarding without implicit relay pairing");
  const onboard =
    await $`PASEO_HOME=${paseoHome} PASEO_LISTEN=127.0.0.1:${port} PASEO_PAIRING_QR=0 npx hanabicode`.nothrow();
  ...
  assert(
    onboard.stdout.includes("hanabicode --help"),
    "onboard output should include --help shortcut",
  );
  assert(onboard.stdout.includes("hanabicode ls"), "onboard output should include ls shortcut");
  assert(
    onboard.stdout.includes('hanabicode run "your prompt"'),
  ...
  const status =
    await $`PASEO_HOME=${paseoHome} npx hanabicode daemon status --home ${paseoHome}`.nothrow();
  ...
  const enableRelay =
    await $`PASEO_HOME=${paseoHome} npx hanabicode daemon pair --home ${paseoHome} --relay`.nothrow();
  ...
  const noRelayOnboard =
    await $`PASEO_HOME=${paseoHome} PASEO_LISTEN=127.0.0.1:${port} npx hanabicode --no-relay`.nothrow();
  assert.strictEqual(
  ...
  await $`PASEO_HOME=${paseoHome} npx hanabicode daemon stop --home ${paseoHome} --force`.nothrow();
