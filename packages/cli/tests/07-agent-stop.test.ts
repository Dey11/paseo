...
try {
  // Test 1: stop --help shows options
  {
    ...
    const result = await $`npx hanabicode stop --help`.nothrow();
  ...
  // Test 2: stop requires ID, --all, or --cwd
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode stop`.nothrow();
  ...
  // Test 3: stop handles daemon not running
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode stop abc123`.nothrow();
  ...
  // Test 4: stop --all flag is accepted
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode stop --all`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: stop --cwd flag is accepted
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode stop --cwd /tmp`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: stop with ID and --host flag is accepted
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode stop abc123 --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: -q (quiet) flag is accepted with stop
  {
    ...
    const result =
      await $`PASEO_HOST=localhost:${port} PASEO_HOME=${paseoHome} npx hanabicode -q stop abc123`.nothrow();
    const output = result.stdout + result.stderr;
