...
function printNextSteps(pairingUrl: string | null, paseoHome: string, richUi: boolean): void {
  ...
  const quickReferenceLines = [
    "1. hanabicode --help",
    "2. hanabicode ls",
    '3. hanabicode run "your prompt"',
    "4. hanabicode daemon status",
...
export async function runOnboard(options: OnboardOptions): Promise<void> {
  ...
  if (richUi) {
    renderNote(paseoHome, "HanabiCode home");
  }
  ...
  if (!pairing.url) {
    ...
    printNextSteps(null, paseoHome, richUi);
    if (richUi) {
      outro("HanabiCode daemon is running.");
    }
    return;
  }
