...
function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/HanabiCode.app",
      path.join(homedir(), "Applications", "HanabiCode.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/HanabiCode",
      "/opt/HanabiCode/HanabiCode",
      path.join(homedir(), "Applications", "HanabiCode.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "HanabiCode", "HanabiCode.exe");
...
function launchDesktop(args: string[]): void {
  if (process.env.PASEO_DESKTOP_CLI === "1") {
    throw new Error(
      "Cannot open HanabiCode Desktop while running in desktop CLI passthrough mode.",
    );
  }
  ...
  if (!desktopApp) {
    throw new Error(
      "HanabiCode desktop app not found. Install it from https://github.com/Dey11/hanabicode/releases",
    );
  }
