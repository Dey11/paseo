...
import { DEFAULT_HANABICODE_LISTEN, loadConfig, resolvePaseoHome } from "@getpaseo/server";
import {
...
const DEFAULT_HOST = DEFAULT_HANABICODE_LISTEN;
...
function resolveConfiguredTcpDaemonHost(env: NodeJS.ProcessEnv, paseoHome: string): string | null {
  ...
  return configuredHost === DEFAULT_HANABICODE_LISTEN ? null : configuredHost;
}
...
export async function connectToDaemon(options?: ConnectOptions): Promise<DaemonClient> {
  ...
  async function tryNext(index: number, lastError: unknown): Promise<DaemonClient> {
    if (index >= hosts.length) {
      ...
      throw new Error(`Unable to connect to HanabiCode daemon via ${hosts.join(", ")}`);
    }
