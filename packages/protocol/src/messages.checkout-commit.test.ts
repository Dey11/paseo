...
describe("checkout commit schemas", () => {
  ...
  test("parses commit-message generation through the session unions", () => {
    const request = {
      type: "checkout.commit_message.generate.request" as const,
      cwd: "/tmp/repo",
      requestId: "generate-1",
    };
    const response = {
      type: "checkout.commit_message.generate.response" as const,
      payload: {
        cwd: "/tmp/repo",
    ...
    expect(CheckoutGenerateCommitMessageResponseSchema.parse(response)).toEqual(response);
    expect(SessionOutboundMessageSchema.parse(response)).toEqual(response);
  });

  test("accepts selective commit and message generation feature flags", () => {
    expect(
